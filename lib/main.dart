...
final softShadow = [BoxShadow(color: const Color(0xFF000000).withValues(alpha: .07), blurRadius: 18, offset: const Offset(0, 7))];

BoxDecoration cardDecoration({Color color = C.card, double radius = 22}) => BoxDecoration(
      color: color,
...
class SprayTarget {
  const SprayTarget(this.id, this.title, this.short, this.color, this.softColor, this.icon);
  final String id;
  final String title;
  final String short;
  final Color color;
...
const sprayTargets = [
  SprayTarget('pest', 'Pest pressure', 'Pest', C.red, C.redSoft, CupertinoIcons.exclamationmark_triangle),
  SprayTarget('fungus', 'Fungal pressure', 'Fungus', C.blue, C.blueSoft, CupertinoIcons.drop),
  SprayTarget('prevent', 'Preventative', 'Prevent', C.forest, C.forestSoft, CupertinoIcons.shield),
  SprayTarget('maintain', 'Plant support', 'Support', C.amber, C.amberSoft, CupertinoIcons.leaf_arrow_circlepath),
];
...
class _SprayTrackerHomeState extends State<SprayTrackerHome> {
  ...
  void _seedBeds() {
    VegetableDefinition byId(String id) => vegetableLibrary.firstWhere((crop) => crop.id == id, orElse: () => vegetableLibrary.first);
  ...
  void removeCrop(int bed, VegetableDefinition crop) {
    final next = [...bedCrops[bed] ?? <VegetableDefinition>[]]..removeWhere((item) => item.id == crop.id);
    setState(() {
      if (next.isEmpty) {
        bedCrops.remove(bed);
      } else {
  ...
  bool bedOnHold(int bed) => records.any((record) => record.beds.contains(bed) && record.onHold);

  int get clearBeds => gardenBeds.where((bed) => !bedOnHold(bed.number)).length;
  int get holdBeds => gardenBeds.length - clearBeds;
  int get plantedBeds => bedCrops.values.where((items) => items.isNotEmpty).length;
  ...
  Widget build(BuildContext context) {
    final pages = [
      ...
      SprayLogScreen(
        ...
        initialCrops: (bedCrops[selectedBed] ?? const <VegetableDefinition>[]).map((crop) => crop.name).toSet(),
        products: products,
        productsLoading: productsLoading,
        onSave: saveSpray,
      ),
      RecordsScreen(records: records, message: message, onDelete: deleteRecord),
      ProductsScreen(products: products, loading: productsLoading, message: message),
    ];
    ...
    return AppPage(
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(color: C.forest, borderRadius: BorderRadius.circular(24), boxShadow: softShadow),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Today', style: TextStyle(color: Color(0xCCFFFFFF), fontSize: 13, fontWeight: FontWeight.w900)),
            const SizedBox(height: 6),
            const Text('Spray status', style: TextStyle(color: CupertinoColors.white, fontSize: 24, fontWeight: FontWeight.w900)),
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: HeroMetric(label: 'CLEAR BEDS', value: '$clearBeds', color: CupertinoColors.white)),
              const SizedBox(width: 10),
              Expanded(child: HeroMetric(label: 'ON HOLD', value: '$holdBeds', color: C.amber)),
            ]),
            const SizedBox(height: 14),
            Text('$plantedBeds beds planted · $cropPlacements crop placements · ${products.length} NZ products', style: const TextStyle(color: Color(0xDFFFFFFF), fontWeight: FontWeight.w700)),
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: CupertinoButton(color: CupertinoColors.white, borderRadius: BorderRadius.circular(16), onPressed: onPlanSpray, child: const Text('Plan a spray', style: TextStyle(color: C.forest, fontWeight: FontWeight.w900)))),
              const SizedBox(width: 10),
              Expanded(child: CupertinoButton(color: const Color(0x18FFFFFF), borderRadius: BorderRadius.circular(16), onPressed: onOpenProducts, child: const Text('Products', style: TextStyle(color: CupertinoColors.white, fontWeight: FontWeight.w900)))),
            ]),
          ]),
        ),
        ...
        if (active.isEmpty) const EmptyCard('No active withholding periods.') else RecordCard(record: active.first),
        const SizedBox(height: 18),
        const SectionTitle('Recent activity'),
        const SizedBox(height: 8),
        if (records.isEmpty) const EmptyCard('No spray records yet. Use Spray Log to test the new product library.') else ...records.take(3).map((record) => RecordCard(record: record)),
      ],
    );
  }
}
...
class HeroMetric extends StatelessWidget {
  const HeroMetric({required this.label, required this.value, required this.color, super.key});
  final String label;
  final String value;
  final Color color;
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(color: const Color(0x12FFFFFF), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0x28FFFFFF))),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: TextStyle(fontSize: 11, color: color, fontWeight: FontWeight.w900)),
          const SizedBox(height: 5),
          Text(value, style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900, color: color)),
        ]),
      );
}

class GardenScreen extends StatelessWidget {
  const GardenScreen({required this.selectedBed, required this.bedCrops, required this.isHold, required this.message, required this.onSelectBed, required this.onAddCrop, required this.onRemoveCrop, required this.onStartSpray, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return AppPage(
      ...
      message: message,
      children: [
        Panel(padding: const EdgeInsets.all(12), child: SizedBox(height: 500, child: GardenMap(selectedBed: selectedBed, bedCrops: bedCrops, isHold: isHold, onTap: onSelectBed))),
        const SizedBox(height: 14),
        Panel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Expanded(child: Text('Bed $selectedBed', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: C.forest))), StatusPill(isHold(selectedBed) ? 'HOLD' : 'CLEAR', hold: isHold(selectedBed))]),
          const SizedBox(height: 4),
          Text(crops.isEmpty ? 'No crops assigned' : crops.map((crop) => crop.name).join(' · '), style: const TextStyle(color: C.muted, fontWeight: FontWeight.w700)),
          const SizedBox(height: 12),
          if (crops.isEmpty) const EmptyInline('Add crops to unlock crop-specific spray logging.') else Wrap(spacing: 8, runSpacing: 8, children: crops.map((crop) => CropChip(crop: crop, onRemove: () => onRemoveCrop(selectedBed, crop))).toList()),
          const SizedBox(height: 14),
          Row(children: [
            Expanded(child: SecondaryButton(label: 'Add crop', onPressed: () => showCropPicker(context, selectedBed, crops, onAddCrop))),
            const SizedBox(width: 10),
            Expanded(child: PrimaryButton(label: 'Spray log', onPressed: onStartSpray)),
          ]),
        ])),
      ],
    );
...
class GardenMap extends StatelessWidget {
  ...
  Widget build(BuildContext context) => LayoutBuilder(builder: (context, constraints) {
        ...
        return Stack(children: [
          ...
          ...gardenBeds.map((bed) {
            final rect = Rect.fromLTWH(bed.rect.left * size.width, bed.rect.top * size.height, bed.rect.width * size.width, bed.rect.height * size.height);
            return Positioned.fromRect(rect: rect, child: BedButton(number: bed.number, selected: selectedBed == bed.number, hold: isHold(bed.number), crops: bedCrops[bed.number] ?? const <VegetableDefinition>[], onTap: () => onTap(bed.number)));
          }),
        ]);
      });
}
  ...
  Widget build(BuildContext context) => CupertinoButton(
        ...
        child: AnimatedContainer(
          ...
          child: Stack(clipBehavior: Clip.none, children: [
            Center(child: Text('$number', style: const TextStyle(color: C.ink, fontWeight: FontWeight.w900, fontSize: 12))),
...
class SprayLogScreen extends StatefulWidget {
  const SprayLogScreen({required this.initialBeds, required this.initialCrops, required this.products, required this.productsLoading, required this.onSave, super.key});
...
class _SprayLogScreenState extends State<SprayLogScreen> {
  ...
  void initState() {
    super.initState();
    if (widget.products.isNotEmpty) _selectProduct(_bestProductForTarget('pest'));
  }
  ...
  SprayProduct _bestProductForTarget(String target) {
    return widget.products.firstWhere((product) => product.targets.contains(target), orElse: () => widget.products.first);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AppPage(
      title: 'Spray Log',
      subtitle: 'Choose product, then withholding and re-entry notes fill automatically.',
      children: [
        const SectionTitle('Beds sprayed'),
        const SizedBox(height: 8),
        Wrap(spacing: 8, runSpacing: 8, children: gardenBeds.map((bed) => NumberChip(label: '${bed.number}', selected: beds.contains(bed.number), onTap: () => setState(() => beds.contains(bed.number) ? beds.remove(bed.number) : beds.add(bed.number)))).toList()),
        const SizedBox(height: 18),
        const SectionTitle('Crops affected'),
        const SizedBox(height: 8),
        if (crops.isEmpty) const EmptyCard('No crops assigned to selected bed yet.') else Wrap(spacing: 8, runSpacing: 8, children: crops.map((crop) => TextChip(label: crop)).toList()),
        const SizedBox(height: 18),
        const SectionTitle('Spraying against'),
        const SizedBox(height: 8),
        TargetGrid(selected: targetId, onSelect: (id) => setState(() { targetId = id; if (widget.products.isNotEmpty) _selectProduct(_bestProductForTarget(id)); })),
        const SizedBox(height: 18),
        const SectionTitle('NZ product library'),
        const SizedBox(height: 8),
        if (widget.productsLoading) const Center(child: CupertinoActivityIndicator()) else if (widget.products.isEmpty) const EmptyCard('No products loaded.') else ...widget.products.map((item) => ProductChoice(product: item, selected: product?.id == item.id, suggested: item.targets.contains(targetId), onTap: () => setState(() => _selectProduct(item)))),
        const SizedBox(height: 18),
        Field(controller: reason, placeholder: 'Issue or reason, e.g. aphids on tomato tips'),
        const SizedBox(height: 8),
        ...
        const SizedBox(height: 18),
        PrimaryButton(label: 'Save spray record', onPressed: product == null || beds.isEmpty ? null : () => widget.onSave(beds: beds, crops: crops, targetId: targetId, product: product, reason: reason.text, notes: notes.text, days: days)),
      ],
    );
  }
}
...
class ProductsScreen extends StatefulWidget {
  const ProductsScreen({required this.products, required this.loading, required this.message, super.key});
  final List<SprayProduct> products;
  final bool loading;
  final String message;
...
class _ProductsScreenState extends State<ProductsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return AppPage(
      title: 'Products',
      subtitle: 'Bundled NZ spray products. Verify labels and ACVM details before use.',
      ...
      children: [
        ...
        const SizedBox(height: 12),
        SectionTitle('NZ spray product library', trailing: Text('${filtered.length}', style: const TextStyle(color: C.muted, fontWeight: FontWeight.w900))),
        const SizedBox(height: 8),
        if (widget.loading) const Center(child: Padding(padding: EdgeInsets.all(24), child: CupertinoActivityIndicator())) else if (filtered.isEmpty) const EmptyCard('No products match that search.') else ...filtered.map((product) => ProductTile(product: product)),
      ],
    );
  }
}
...
class RecordsScreen extends StatelessWidget {
  const RecordsScreen({required this.records, required this.message, required this.onDelete, super.key});
  final List<SprayRecord> records;
  final String message;
  ...
  Widget build(BuildContext context) => AppPage(
        ...
        message: message,
        children: [
          if (records.isEmpty) const EmptyCard('No spray records yet.') else ...records.map((record) => RecordCard(record: record, onDelete: () => onDelete(record.id))),
        ],
      );
}
...
class ProductTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CupertinoButton(
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text(product.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: C.ink, fontWeight: FontWeight.w900, fontSize: 16))),
            ...
            if (product.commonUses.isNotEmpty) ...[
              const SizedBox(height: 8),
              Text('Uses: ${product.commonUses.take(5).join(', ')}', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: C.ink, fontSize: 12, fontWeight: FontWeight.w700)),
            ],
          ]),
        ),
      );
        ...
        child: Container(
          ...
          child: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(product.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: C.ink, fontWeight: FontWeight.w900)),
              Text('${product.type} · ${product.withholdingDays} day WHP · Re-entry ${product.reEntryHours} hr', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: C.muted, fontSize: 12, fontWeight: FontWeight.w700)),
            ])),
...
class SprayProductHelperNotes extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('Re-entry: ${product.reEntryHours} hr', style: const TextStyle(color: C.muted, fontWeight: FontWeight.w700, fontSize: 12)),
          if (product.reSprayIntervalDays > 0) Text('Re-spray interval: ${product.reSprayIntervalDays} days', style: const TextStyle(color: C.muted, fontWeight: FontWeight.w700, fontSize: 12)),
        ]),
      );
}
...
void showCropPicker(BuildContext context, int bed, List<VegetableDefinition> assigned, void Function(int bed, VegetableDefinition crop) onAdd) {
  showCupertinoModalPopup<void>(context: context, builder: (_) => Sheet(child: ListView(padding: const EdgeInsets.all(20), children: [SheetHeader(title: 'Add crop', subtitle: 'Bed $bed'), const SizedBox(height: 12), ...vegetableLibrary.map((crop) { final added = assigned.any((item) => item.id == crop.id); return CupertinoButton(padding: EdgeInsets.zero, onPressed: added ? null : () { onAdd(bed, crop); Navigator.pop(context); }, child: Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(12), decoration: cardDecoration(), child: Row(children: [CropIcon(crop.iconPath, size: 38), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(crop.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: C.ink, fontWeight: FontWeight.w900)), Text(crop.familyId, style: const TextStyle(color: C.muted, fontSize: 12, fontWeight: FontWeight.w700))])), StatusPill(added ? 'ADDED' : 'ADD', hold: false)]))); })])));
}
...
class BottomNav extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: C.card, borderRadius: BorderRadius.circular(31), border: Border.all(color: C.line), boxShadow: softShadow),
        ...
        return Expanded(
          child: CupertinoButton(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              AnimatedContainer(duration: const Duration(milliseconds: 160), width: 38, height: 34, alignment: Alignment.center, decoration: BoxDecoration(color: selected ? C.forest : CupertinoColors.transparent, borderRadius: BorderRadius.circular(999)), child: Icon(items[index].icon, size: 18, color: selected ? CupertinoColors.white : C.muted)),
              const SizedBox(height: 3),
              FittedBox(child: Text(items[index].label, style: TextStyle(color: selected ? C.forest : C.muted, fontSize: 9.5, fontWeight: FontWeight.w800))),
            ]),
          ),
        );
      })),
...
class RecordCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: cardDecoration(),
      child: Row(children: [
        Container(width: 44, height: 44, alignment: Alignment.center, decoration: BoxDecoration(color: target.softColor, borderRadius: BorderRadius.circular(14)), child: Icon(target.icon, color: target.color, size: 21)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Bed ${record.beds.join(', ')} · ${target.short}', style: const TextStyle(fontWeight: FontWeight.w900, color: C.ink)),
          ...
          Text('${record.product} · sprayed ${shortDate(record.date)} · safe ${shortDate(record.safeDate)}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: C.muted, fontSize: 12)),
        ])),
        StatusPill(record.onHold ? 'HOLD' : 'SAFE', hold: record.onHold),
        if (onDelete != null) CupertinoButton(padding: EdgeInsets.zero, minimumSize: const Size(32, 32), onPressed: onDelete, child: const Text('×', style: TextStyle(color: C.red, fontSize: 22, fontWeight: FontWeight.w900))),
      ]),
    );
  }
}
...
class AppPage extends StatelessWidget {
  const AppPage({required this.title, required this.subtitle, required this.children, this.message = '', super.key});
  final String title;
  ...
  @override
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.fromLTRB(20, 18, 20, 26), children: [
        Text(title, style: const TextStyle(fontSize: 34, fontWeight: FontWeight.w900, letterSpacing: -1.1, color: C.forest)),
        const SizedBox(height: 4),
        Text(subtitle, style: const TextStyle(fontSize: 14, color: C.muted, fontWeight: FontWeight.w700)),
        const SizedBox(height: 18),
...
class Panel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: padding, decoration: cardDecoration(), child: child);
}
...
class SectionTitle extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Row(children: [Expanded(child: Text(text, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: C.forest))), if (trailing != null) trailing!]);
}
...
class StatusPill extends StatelessWidget {
  ...
  final bool hold;
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: hold ? C.amberSoft : C.forestSoft, borderRadius: BorderRadius.circular(999)), child: Text(label, style: TextStyle(color: hold ? C.amber : C.forest, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: .5)));
}
...
class ProductTag extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: background, borderRadius: BorderRadius.circular(999)), child: Text(label, style: TextStyle(color: color, fontWeight: FontWeight.w900, fontSize: 10.5)));
}
...
class CategoryPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final background = lower == 'organic' ? C.forestSoft : lower == 'chemical' ? C.amberSoft : C.greySoft;
...
class Stepper extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: C.soft, borderRadius: BorderRadius.circular(16), border: Border.all(color: C.line)), child: Row(children: [Expanded(child: Text(label, style: const TextStyle(fontWeight: FontWeight.w900))), SmallButton('-', minus), Padding(padding: const EdgeInsets.symmetric(horizontal: 14), child: Text('$value', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900))), SmallButton('+', plus)]));
}
...
class SmallButton extends StatelessWidget {
  ...
  final VoidCallback? onPressed;
  @override
  Widget build(BuildContext context) => CupertinoButton(padding: EdgeInsets.zero, minimumSize: const Size(34, 34), color: C.card, borderRadius: BorderRadius.circular(999), onPressed: onPressed, child: Text(label, style: const TextStyle(color: C.forest, fontSize: 18, fontWeight: FontWeight.w900)));
}
...
class CropChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.fromLTRB(10, 8, 6, 8), decoration: BoxDecoration(color: C.card, borderRadius: BorderRadius.circular(999), border: Border.all(color: C.line)), child: Row(mainAxisSize: MainAxisSize.min, children: [CropIcon(crop.iconPath, size: 22), const SizedBox(width: 7), Text(crop.name, style: const TextStyle(color: C.ink, fontWeight: FontWeight.w900, fontSize: 13)), CupertinoButton(padding: EdgeInsets.zero, minimumSize: Size.zero, onPressed: onRemove, child: const Text('×', style: TextStyle(fontSize: 18, color: C.muted)))]));
}
...
class DetailLine extends StatelessWidget {
  ...
  final String value;
  @override
  Widget build(BuildContext context) => Padding(padding: const EdgeInsets.only(bottom: 10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(color: C.forest, fontWeight: FontWeight.w900, fontSize: 12)), const SizedBox(height: 2), Text(value.isEmpty ? '—' : value, style: const TextStyle(color: C.ink, fontWeight: FontWeight.w700, height: 1.25))]));
}
...
class TargetGrid extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => GridView.count(crossAxisCount: 4, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisSpacing: 8, mainAxisSpacing: 8, childAspectRatio: .98, children: sprayTargets.map((target) => TargetButton(target: target, selected: selected == target.id, onTap: () => onSelect(target.id))).toList());
}
...
class TargetButton extends StatelessWidget {
  ...
  final VoidCallback onTap;
  @override
  Widget build(BuildContext context) => CupertinoButton(padding: EdgeInsets.zero, onPressed: onTap, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8), decoration: BoxDecoration(color: selected ? target.softColor : C.card, borderRadius: BorderRadius.circular(16), border: Border.all(color: selected ? target.color : C.line, width: selected ? 1.8 : 1)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(target.icon, color: target.color, size: 22), const SizedBox(height: 5), FittedBox(child: Text(target.short, style: const TextStyle(color: C.ink, fontSize: 11, fontWeight: FontWeight.w900)))])));
}
