...
const cropProfiles = [
  CropProfile(
    ...
    fungusPressure: 'Blight, powdery mildew, leaf spot',
    preventative: ['Inspect undersides weekly', 'Use copper only when fungal risk is high', 'Avoid wetting foliage late in the day'],
    maintenance: ['Prune lower leaves', 'Improve airflow', 'Remove infected leaves quickly'],
  ),
  CropProfile(
    name: 'Leafy greens',
    iconPath: 'assets/veg_icons/leafy.svg',
    pestPressure: 'Aphids, slugs, caterpillars',
    fungusPressure: 'Downy mildew, damping off, leaf spot',
    preventative: ['Use insect netting where possible', 'Use soft pest sprays only when pests are present', 'Keep foliage dry where practical'],
    maintenance: ['Thin crowded plants', 'Remove damaged leaves', 'Harvest outer leaves often'],
  ),
  CropProfile(
    name: 'Onions / alliums',
    ...
    preventative: ['Monitor for thrips in dry weather', 'Avoid excess nitrogen', 'Keep beds weed free'],
  ...
  CropProfile(
    ...
    preventative: ['Keep fruit off wet soil', 'Use copper only during dormant or label-approved windows', 'Net fruit before ripening'],
    maintenance: ['Prune crowded canes', 'Remove old fruit', 'Improve airflow'],
  ),
  CropProfile(
    ...
    preventative: ['Avoid overwatering', 'Use fine mesh for carrot fly pressure', 'Keep mulch light near crowns'],
    maintenance: ['Thin early', 'Keep soil evenly moist', 'Rotate beds between seasons'],
  ),
];
...
class _SprayTrackerHomeState extends State<SprayTrackerHome> {
  ...
  final Map<int, CropProfile> bedCrops = {};
  ...
  void setCropForBed(int bedNumber, CropProfile crop) {
    setState(() {
      bedCrops[bedNumber] = crop;
      selectedBed = bedNumber;
      actionMessage = '${crop.name} assigned to Bed $bedNumber';
    });
  }

  void clearCropForBed(int bedNumber) {
    setState(() {
      bedCrops.remove(bedNumber);
      selectedBed = bedNumber;
      actionMessage = 'Crop cleared from Bed $bedNumber';
    });
  }
  ...
  Widget build(BuildContext context) {
    final pages = [
      DashboardScreen(safeCount: safeCount, waitCount: waitCount, records: records, waitingRecords: waitingRecords, cropCount: bedCrops.length, actionMessage: actionMessage, onOpenLog: () => openLog(selectedBeds)),
      VisualMapScreen(selectedBed: selectedBed, crops: bedCrops, actionMessage: actionMessage, bedStatus: statusForBed, onSelectBed: selectBedOnMap, onSetCrop: setCropForBed, onClearCrop: clearCropForBed, onLogBed: (bedNumber) => openLog({bedNumber}), onClearSprays: clearBedSprays),
      LogSprayScreen(key: ValueKey('${selectedBeds.join(',')}-${products.length}-${bedCrops.length}'), initialBeds: selectedBeds, products: products, crops: bedCrops, onSave: saveSpray),
...
class DashboardScreen extends StatelessWidget {
  const DashboardScreen({required this.safeCount, required this.waitCount, required this.records, required this.waitingRecords, required this.cropCount, required this.actionMessage, required this.onOpenLog, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return AppPage(title: 'Spray Tracker', subtitle: 'Spray records, withholding periods, and bed status.', children: [
      ...
      DashboardHero(safeCount: safeCount, waitCount: waitCount, cropCount: cropCount),
...
class DashboardHero extends StatelessWidget {
  const DashboardHero({required this.safeCount, required this.waitCount, required this.cropCount, super.key});
  final int safeCount;
  final int waitCount;
  final int cropCount;
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: AppColor.primary, borderRadius: BorderRadius.circular(24), boxShadow: shadow), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Text('$cropCount beds assigned crops', style: const TextStyle(color: Color(0xEFFFFFFF), fontWeight: FontWeight.w700)),
  ]));
}
...
class VisualMapScreen extends StatelessWidget {
  const VisualMapScreen({required this.selectedBed, required this.crops, required this.actionMessage, required this.bedStatus, required this.onSelectBed, required this.onSetCrop, required this.onClearCrop, required this.onLogBed, required this.onClearSprays, super.key});

  final int selectedBed;
  final Map<int, CropProfile> crops;
  final String? actionMessage;
  final BedStatus Function(int) bedStatus;
  final ValueChanged<int> onSelectBed;
  final void Function(int bedNumber, CropProfile crop) onSetCrop;
  final ValueChanged<int> onClearCrop;
  final ValueChanged<int> onLogBed;
  final ValueChanged<int> onClearSprays;

  @override
  Widget build(BuildContext context) {
    final crop = crops[selectedBed];
    final status = bedStatus(selectedBed);
    return AppPage(title: 'Garden Map', subtitle: 'Assign crops and view pressure guidance by bed.', children: [
      if (actionMessage != null) ...[ActionBanner(message: actionMessage!), const SizedBox(height: 12)],
      GardenVisualMap(selectedBed: selectedBed, crops: crops, bedStatus: bedStatus, onSelectBed: onSelectBed),
      const SizedBox(height: 14),
      SelectedBedPanel(bedNumber: selectedBed, crop: crop, status: status, onAddCrop: () => showCropPicker(context, selectedBed, onSetCrop), onClearCrop: crop == null ? null : () => onClearCrop(selectedBed), onLog: () => onLogBed(selectedBed), onClearSprays: () => onClearSprays(selectedBed)),
      if (crop != null) ...[const SizedBox(height: 14), SprayGuidancePanel(crop: crop)],
    ]);
  }
}

class GardenVisualMap extends StatelessWidget {
  const GardenVisualMap({required this.selectedBed, required this.crops, required this.bedStatus, required this.onSelectBed, super.key});
  final int selectedBed;
  final Map<int, CropProfile> crops;
  final BedStatus Function(int) bedStatus;
  final ValueChanged<int> onSelectBed;
  ...
  Widget build(BuildContext context) => Container(height: 560, padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColor.surface, borderRadius: BorderRadius.circular(22), border: Border.all(color: AppColor.line), boxShadow: subtleShadow), child: LayoutBuilder(builder: (context, constraints) {
    ...
    return Stack(children: [
      ...
      ...gardenBeds.map((bed) => MapBedTile(bed: bed, rect: scaleRect(bed.bounds, size), selected: bed.number == selectedBed, crop: crops[bed.number], status: bedStatus(bed.number), onTap: () => onSelectBed(bed.number))),
    ]);
  }));
}

class MapBedTile extends StatelessWidget {
  const MapBedTile({required this.bed, required this.rect, required this.selected, required this.crop, required this.status, required this.onTap, super.key});
  final GardenBed bed;
  final Rect rect;
  final bool selected;
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned.fromRect(rect: rect, child: CupertinoButton(padding: EdgeInsets.zero, onPressed: onTap, child: Stack(clipBehavior: Clip.none, children: [
      AnimatedContainer(duration: const Duration(milliseconds: 140), alignment: Alignment.center, decoration: BoxDecoration(color: waiting ? AppColor.warningSoft : crop == null ? const Color(0xFFFCFBF7) : AppColor.primarySoft, borderRadius: BorderRadius.circular(7), border: Border.all(color: selected ? CupertinoColors.activeBlue : waiting ? AppColor.warning : AppColor.earth, width: selected ? 3 : 1.7)), child: Text('${bed.number}', style: const TextStyle(color: AppColor.ink, fontSize: 13, fontWeight: FontWeight.w900))),
      if (crop != null) Positioned(top: -8, right: -8, child: HoverCropIcon(crop: crop!)),
    ])));
  }
}
...
class SprayGuidancePanel extends StatelessWidget {
  const SprayGuidancePanel({required this.crop, super.key});
  final CropProfile crop;
  @override
  Widget build(BuildContext context) => PremiumCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(children: [SizedBox(width: 36, height: 36, child: SvgPicture.asset(crop.iconPath)), const SizedBox(width: 10), Expanded(child: Text('${crop.name} guidance', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900)))]),
    const SizedBox(height: 14),
    GuidanceBlock(title: 'Pest pressure', body: crop.pestPressure, icon: CupertinoIcons.ant),
    GuidanceBlock(title: 'Fungal pressure', body: crop.fungusPressure, icon: CupertinoIcons.cloud_rain),
    GuidanceList(title: 'Preventative spray considerations', items: crop.preventative),
    GuidanceList(title: 'Maintenance tasks', items: crop.maintenance),
    const SizedBox(height: 6),
    const Text('Use product labels and local withholding guidance before applying any spray.', style: TextStyle(color: AppColor.muted, fontSize: 12, fontWeight: FontWeight.w600)),
  ]));
}

class GuidanceBlock extends StatelessWidget {
  const GuidanceBlock({required this.title, required this.body, required this.icon, super.key});
  final String title;
  final String body;
  final IconData icon;
  @override
  Widget build(BuildContext context) => Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, size: 18, color: AppColor.primary), const SizedBox(width: 8), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.w900)), Text(body, style: const TextStyle(color: AppColor.muted, fontSize: 13))]))]));
}

class GuidanceList extends StatelessWidget {
  const GuidanceList({required this.title, required this.items, super.key});
  final String title;
  final List<String> items;
  @override
  Widget build(BuildContext context) => Padding(padding: const EdgeInsets.only(bottom: 10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.w900)), const SizedBox(height: 4), ...items.map((item) => Padding(padding: const EdgeInsets.only(bottom: 3), child: Text('• $item', style: const TextStyle(color: AppColor.muted, fontSize: 13))))]));
}

void showCropPicker(BuildContext context, int bedNumber, void Function(int, CropProfile) onSave) {
  showCupertinoModalPopup<void>(context: context, builder: (_) => CupertinoActionSheet(
    title: Text('Assign crop to Bed $bedNumber'),
    message: const Text('Choose a crop profile. The icon will appear on the map and pressure guidance will update.'),
    actions: cropProfiles.map((crop) => CupertinoActionSheetAction(onPressed: () { onSave(bedNumber, crop); Navigator.pop(context); }, child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [SizedBox(width: 24, height: 24, child: SvgPicture.asset(crop.iconPath)), const SizedBox(width: 10), Text(crop.name)]))).toList(),
...
class SelectedBedPanel extends StatelessWidget {
  const SelectedBedPanel({required this.bedNumber, required this.crop, required this.status, required this.onAddCrop, required this.onClearCrop, required this.onLog, required this.onClearSprays, super.key});
  final int bedNumber;
  final CropProfile? crop;
  final BedStatus status;
  ...
  Widget build(BuildContext context) => PremiumCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Text(crop == null ? 'Empty bed' : crop!.name, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
    const SizedBox(height: 4),
    Text(status.summary, style: const TextStyle(color: AppColor.muted)),
    const SizedBox(height: 14),
    Row(children: [Expanded(child: PrimaryButton(title: crop == null ? 'Assign crop' : 'Change crop', onPressed: onAddCrop)), const SizedBox(width: 10), Expanded(child: SecondaryButton(title: 'Log spray', icon: CupertinoIcons.drop_fill, onPressed: onLog))]),
    const SizedBox(height: 10),
    Row(children: [Expanded(child: DestructiveButton(title: 'Clear spray', onPressed: onClearSprays)), const SizedBox(width: 10), Expanded(child: DestructiveButton(title: 'Clear crop', onPressed: onClearCrop))]),
  ]));
}

class LogSprayScreen extends StatefulWidget {
  const LogSprayScreen({required this.initialBeds, required this.products, required this.crops, required this.onSave, super.key});
...
class _LogSprayScreenState extends State<LogSprayScreen> {
  ...
  Widget build(BuildContext context) => AppPage(title: 'Log Spray', subtitle: ['Select beds', 'Select product', 'Add details', 'Review and save'][step], leading: step == 0 ? null : CupertinoButton(padding: EdgeInsets.zero, onPressed: goBack, child: const Icon(CupertinoIcons.back, color: AppColor.ink)), children: [
    ...
    if (step == 0) SelectBedsStep(selectedBeds: selectedBeds, crops: widget.crops, onChanged: (beds) => setState(() => selectedBeds = beds)),
...
class SelectBedsStep extends StatelessWidget {
  const SelectBedsStep({required this.selectedBeds, required this.crops, required this.onChanged, super.key});
  final Set<int> selectedBeds;
  final Map<int, CropProfile> crops;
  final ValueChanged<Set<int>> onChanged;
  @override
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const SectionHeader(title: 'Select beds'), const SizedBox(height: 6), Text('${selectedBeds.length} selected', style: const TextStyle(color: AppColor.muted, fontWeight: FontWeight.w700)), const SizedBox(height: 12), Wrap(spacing: 8, runSpacing: 8, children: gardenBeds.map((bed) => BedChip(number: bed.number, selected: selectedBeds.contains(bed.number), iconPath: crops[bed.number]?.iconPath, onTap: () { final next = {...selectedBeds}; next.contains(bed.number) ? next.remove(bed.number) : next.add(bed.number); onChanged(next); })).toList())]);
}
